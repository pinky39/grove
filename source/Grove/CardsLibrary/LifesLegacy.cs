  ...
  public class LifesLegacy : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{1}{G}")
        .Type("Sorcery")
        .Text("As an additional cost to cast Life's Legacy, sacrifice a creature.{EOL}Draw cards equal to the sacrificed creature's power.")
        .FlavorText("At the instant of death, the mystery of life.")
        .Cast(p =>
        {
          p.Cost = new AggregateCost(
            new PayMana("{1}{G}".Parse(), ManaUsage.Spells),
            new Sacrifice());
