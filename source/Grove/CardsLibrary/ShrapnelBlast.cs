    ...
    public class ShrapnelBlast : CardTemplateSource
    {
        public override IEnumerable<CardTemplate> GetCards()
        {
            yield return Card
                ...
                .ManaCost("{1}{R}")
                .Type("Instant")
                .Text("As an additional cost to cast Shrapnel Blast, sacrifice an artifact.{EOL}Shrapnel Blast deals 5 damage to target creature or player.")
                .FlavorText("It requires immense pressure to imbue an item with magic. If the spell is broken...")
                .Cast(p =>
                {
                    p.Cost = new AggregateCost(
                        new PayMana("{1}{R}".Parse(), ManaUsage.Spells),
                        new Sacrifice());
