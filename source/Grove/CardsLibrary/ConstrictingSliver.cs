    ...
    public class ConstrictingSliver : CardTemplateSource
    {
        public override IEnumerable<CardTemplate> GetCards()
        {
            yield return Card
                ...
                .ContinuousEffect(p =>
                {
                    p.CardFilter = (card, effect) => card.Controller == effect.Source.Controller && card.Is().Creature && card.Is().OfType("Sliver");
                    p.Modifier = () =>
                    {
                        var tp = new TriggeredAbility.Parameters();

                        tp.Text = "When this creature enters the battlefield, you may exile target creature an opponent controls until this creature leaves the battlefield.";
